// 1. CONNECT TO THE NEW MOCKAPI DATABASE
const API_URL = "https://69ba5f18b3dcf7e0b4bcb73f.mockapi.io/products";

// 2. Products array starts EMPTY. The database will fill it dynamically!
let products = []; 
...
document.addEventListener("DOMContentLoaded", () => {
    restoreSession(); 
    fetchProductsFromDatabase(); // Call the database on load
    startAutoSlide();
});

// Fetch data directly from MockAPI
async function fetchProductsFromDatabase() {
    try {
        const response = await fetch(API_URL);
        const data = await response.json();
        
        // Convert the database rows into the product format
        ...
        if(grid) grid.innerHTML = "<p style='text-align:center; width:100%; color:red;'>Failed to load products. Please check your connection.</p>";
    }
}
...
function renderProducts() {
    ...
    // If database is empty or hasn't loaded yet
    if (products.length === 0) {
        if(homeGrid) homeGrid.innerHTML = '<p style="text-align:center; width:100%;">Database is empty. Please upload products via the Admin Panel!</p>';
        if(allProductsGrid) allProductsGrid.innerHTML = '<p style="text-align:center; width:100%;">No products found.</p>';
        return;
    }
